...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12 font-sans transform-gpu">
      {/* Welcome Banner */}
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-500 rounded-3xl p-6 md:p-8 text-white flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl shadow-blue-500/15 border border-blue-400/30">
        <div className="space-y-2 relative z-10">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-white/20 text-white border border-white/30 text-[10px] font-extrabold tracking-wide uppercase flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-300 animate-pulse"></span>
              Văn phòng Tham mưu Cấp ủy
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-500/30 text-emerald-100 border border-emerald-300/40 text-[10px] font-bold tracking-wide uppercase flex items-center gap-1">
              <HardDrive className="w-3 h-3 text-emerald-300" />
              Drive Sync Active
            </span>
          </div>
          ...
          <p className="text-xs md:text-sm text-blue-50 max-w-2xl leading-relaxed font-normal">
            Hệ thống ứng dụng AI Gemini để tự động phân tích thẩm quyền xử lý, đề xuất phân luồng (Ban Thường vụ / UBND / Sở Ban Ngành), tự động lưu trữ vào Google Drive và lập Phiếu Trình điện tử.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 relative z-10">
          <Link
            to="/documents"
            className="px-4 py-2.5 bg-white/15 hover:bg-white/25 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 border border-white/20 shadow-md active:scale-95"
          >
          ...
          <Link
            to="/tasks"
            className="px-4 py-2.5 bg-white hover:bg-blue-50 text-blue-700 rounded-xl text-xs font-extrabold transition-all flex items-center gap-1.5 shadow-lg shadow-black/10 active:scale-95 group"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-blue-100/80 shadow-xs flex items-center gap-4 hover:border-blue-300 hover:shadow-md transition-all group">
          <div className="w-12 h-12 bg-blue-50 border border-blue-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 group-hover:bg-blue-600 group-hover:text-white transition-all text-blue-600 shadow-2xs">
            <FileText className="w-6 h-6 transition-colors" />
          </div>
          <div>
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tổng văn bản</div>
            <div className="text-2xl font-black text-slate-900 leading-none mt-1">{stats.totalDocs}</div>
          </div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-blue-100/80 shadow-xs flex items-center gap-4 hover:border-blue-300 hover:shadow-md transition-all group">
          <div className="w-12 h-12 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 group-hover:bg-indigo-600 group-hover:text-white transition-all text-indigo-600 shadow-2xs">
            <ShieldAlert className="w-6 h-6 transition-colors" />
          </div>
          <div>
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Trình Ban Thường vụ</div>
            <div className="text-2xl font-black text-indigo-600 leading-none mt-1">{stats.standingBoardDocs}</div>
          </div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-blue-100/80 shadow-xs flex items-center gap-4 hover:border-blue-300 hover:shadow-md transition-all group">
          <div className="w-12 h-12 bg-sky-50 border border-sky-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 group-hover:bg-sky-600 group-hover:text-white transition-all text-sky-600 shadow-2xs">
            <HardDrive className="w-6 h-6 transition-colors" />
          </div>
          <div>
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Lưu Google Drive</div>
            <div className="text-2xl font-black text-sky-600 leading-none mt-1">{stats.driveSyncedDocs}</div>
          </div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-blue-100/80 shadow-xs flex items-center gap-4 hover:border-blue-300 hover:shadow-md transition-all group">
          <div className="w-12 h-12 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 group-hover:bg-emerald-600 group-hover:text-white transition-all text-emerald-600 shadow-2xs">
            <CheckCircle2 className="w-6 h-6 transition-colors" />
          </div>
          <div>
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Nhiệm vụ đang đôn đốc</div>
            <div className="text-2xl font-black text-slate-900 leading-none mt-1">{stats.pendingTasks}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xs space-y-4">
            ...
            <div 
              ...
              className={`
                ...
                ${isDragActive ? 'border-blue-500 bg-blue-50/70 ring-2 ring-blue-200' : 'border-slate-300/80 hover:border-blue-400 bg-slate-50/70 hover:bg-blue-50/30'}
              `}
            >
              <input {...getInputProps()} />
              <div className="flex flex-col items-center justify-center space-y-3">
                {isUploading ? (
                  ...
                  <>
                    <div className="w-14 h-14 bg-gradient-to-tr from-blue-50 to-indigo-50 border border-blue-200/60 text-blue-600 rounded-2xl flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[11px] text-slate-400 font-semibold uppercase tracking-wider">
                        PDF, DOCX (Dung lượng tối đa 15MB)
                      </p>
                    </div>
                  </>
                )}
            ...
            {error && (
              <div className="p-3.5 bg-red-50 text-red-700 rounded-lg flex items-start gap-2.5 text-xs border border-red-200">
          ...
          <div className="bg-gradient-to-br from-blue-50 via-slate-50 to-indigo-50/60 rounded-2xl p-5 border border-blue-200 shadow-2xs space-y-3.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wide">Lưu trữ Google Drive</h3>
                  <p className="text-[11px] text-blue-700 font-medium">Tự động đồng bộ tệp gốc khi tải lên</p>
                </div>
              </div>
              ...
              {hasDriveToken ? (
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-bold rounded-lg transition-all flex items-center gap-1 shadow-2xs disabled:opacity-50"
            ...
            <div className="pt-1 flex flex-col sm:flex-row gap-2">
              <a
                ...
                className="flex-1 py-2 px-3 bg-white hover:bg-blue-600 hover:text-white text-blue-700 border border-blue-200 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-2xs group"
              >
                <span>Mở Thư mục Drive Cơ quan</span>
              ...
              {!hasDriveToken && (
                <button
                  onClick={handleConnectDrive}
                  disabled={isConnectingDrive}
                  className="py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-2xs disabled:opacity-50"
                >
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-2xs space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Quy trình thẩm định tự động</h3>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xs border border-slate-200 overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Văn bản Mới Tiếp Nhận & Phân Luồng</h3>
                <p className="text-xs text-slate-400 mt-0.5">Danh sách văn bản đã được thẩm định tự động & đồng bộ</p>
              </div>
              ...
              {recentDocs.length === 0 ? (
                ...
                recentDocs.map((doc) => (
                  <div
                    ...
                    className="p-4 hover:bg-blue-50/30 cursor-pointer transition-colors flex items-center justify-between gap-4 group"
                  >
                    <div className="flex items-start gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {doc.proposedAction && (
                            <span className="px-2 py-0.5 bg-blue-50 text-blue-800 border border-blue-200 text-[10px] font-bold rounded-md truncate max-w-[220px]">
                              {doc.proposedAction}
                            </span>
                          )}
                          {doc.leadDepartment && (
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-[10px] font-medium rounded-md truncate max-w-[150px]">
                        ...
                        <h4 className="text-xs font-semibold text-slate-800 line-clamp-1 group-hover:text-blue-700 transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-2xs border border-slate-200 overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Nhiệm vụ Chỉ Đạo Gần Đây</h3>
                <p className="text-xs text-slate-400 mt-0.5">Tiến độ thực hiện nhiệm vụ bóc tách từ văn bản</p>
              </div>
